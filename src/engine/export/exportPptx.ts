...
function addSlide(
  pSlide: PptxGenJS['addSlide'] extends (...a: any[]) => infer R ? R : never,
  slide: Slide,
  t: Theme,
  meta: Meta,
) {
  ...
  switch (slide.layout) {
    ...
    default:              addTitleContentSlide(s, slide, t, cy, ch);
  }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type PS = any; // pptxgenjs Slide instance
...
function addTitleContentSlide(s: PS, slide: Slide, t: Theme, cy: number, ch: number) {
  ...
  addElements(s, slide.elements, t, { x: M, y: cy + hh + 0.1, w: W - M * 2, h: ch - hh - 0.1 });
}

function addTitleImageSlide(s: PS, slide: Slide, t: Theme, cy: number, ch: number) {
  s.background = { fill: hex(t.colors.background) };
  ...
  if (img && img.type === 'image') {
    tryAddImage(s, img.src, { x: imgX, y: cy, w: imgW, h: ch });
  }
}

function addSplitSlide(s: PS, slide: Slide, t: Theme, cy: number, ch: number) {
  s.background = { fill: hex(t.colors.background) };
  ...
  if (img && img.type === 'image') {
    tryAddImage(s, img.src, { x: M, y: bodyY, w: colW, h: bodyH });
  }
...
function addFullBleedSlide(s: PS, slide: Slide, t: Theme) {
  ...
  const img = slide.elements.find((e) => e.type === 'image');
  if (img && img.type === 'image') {
    tryAddImage(s, img.src, { x: 0, y: 0, w: W, h: H });
  }
}
...
function addTwoColumnSlide(s: PS, slide: Slide, t: Theme, cy: number, ch: number) {
  s.background = { fill: hex(t.colors.background) };
  const hh = slide.title ? 0.65 : 0;
  if (slide.title) {
    s.addText(slide.title, {
  ...
  addElements(s, left,  t, { x: M,               y: bodyY, w: colW, h: bodyH });
  addElements(s, right, t, { x: M + colW + 0.3,  y: bodyY, w: colW, h: bodyH });
}

function addGridSlide(s: PS, slide: Slide, t: Theme, cy: number, ch: number) {
  s.background = { fill: hex(t.colors.background) };
  const hh = slide.title ? 0.65 : 0;
  if (slide.title) {
    s.addText(slide.title, {
...
function addElements(s: PS, elements: SlideElement[], t: Theme, area: Area) {
  if (elements.length === 0) return;

  // Single image fills the area
  if (elements.length === 1 && elements[0].type === 'image') {
    tryAddImage(s, elements[0].src, area);
    return;
  }
  ...
  // Tables: placed after text (rough split)
  const tableEl = elements.find((e) => e.type === 'table');
  if (tableEl && tableEl.type === 'table') {
    const tableY = runs.length > 0 ? area.y + area.h * 0.45 : area.y;
    const tableH = runs.length > 0 ? area.h * 0.52 : area.h;
...
function tryAddImage(s: PS, src: string, area: Area) {
  if (!src) return;
  // asset:// and tauri:// are Tauri internal protocols, not embeddable by pptxgenjs
  if (src.startsWith('asset://') || src.startsWith('tauri://')) return;
  if (src.startsWith('http://') || src.startsWith('https://')) {
    try {
      s.addImage({ path: src, x: area.x, y: area.y, w: area.w, h: area.h });
    } catch { /* skip unfetchable images */ }
  }
  // Local relative/absolute paths are skipped in browser mode
}
